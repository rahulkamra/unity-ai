...
namespace LlmTornado.Chat.Vendors.Google
{
    ...
    internal class VendorGoogleChatRequest
    {
        ...
        {
            [JsonProperty("parts")]
            public List<VendorGoogleChatRequestMessagePart>? Parts { get; set; } = new List<VendorGoogleChatRequestMessagePart>();
            ...
            public VendorGoogleChatRequestMessage(ChatMessage msg, VendorGoogleChatRequest? request)
            {
                if (msg is { Role: ChatMessageRoles.Assistant, ToolCalls: { Count: > 0 } })
                {
                    foreach (ToolCall call in msg.ToolCalls)
                    {
                        ...
                        Parts ??= new List<VendorGoogleChatRequestMessagePart>();
                        Parts.Add(new VendorGoogleChatRequestMessagePart
                        {
                            FunctionCall = new VendorGoogleChatRequestMessagePartFunctionCall
                            {
                                ...
                                Args = call.FunctionCall?.GetArguments() ?? new Dictionary<string, object?>()
                            },
                            ThoughtSignature = thoughtSignature
                        });
                    }
                ...
                else if (msg.Role is ChatMessageRoles.Tool)
                {
                    ...
                    if (msg.FunctionCall?.Result?.RawContentBlocks is not null)
                    {
                        ...
                        foreach (IFunctionResultBlock block in msg.FunctionCall.Result.RawContentBlocks)
                        {
                            ...
                            {
                                responseParts ??= new List<VendorGoogleChatRequestMessagePartFunctionResponsePart>();
            ...
            public ChatMessage ToChatMessage(VendorGoogleChatRequest? request, ChatRequest? chatRequest, VendorGoogleChatResult.VendorGoogleChatResultMessage nativeResult)
            {
                ...
                foreach (VendorGoogleChatRequestMessagePart x in Parts ?? new List<VendorGoogleChatRequestMessagePart>())
                {
                ...
                if (nativeResult.GroundingMetadata?.GroundingSupports.Count > 0)
                {
                    ...
                    {
                        lastPart.Citations ??= new List<IChatMessagePartCitation>();
        ...
        internal class VendorGoogleChatRequestSafetySetting
        {
            ...
            /// </summary>
            internal static readonly List<string> AllCategories = new List<string>
            {
            ...
            internal static readonly List<VendorGoogleChatRequestSafetySetting> DisableAll = new List<VendorGoogleChatRequestSafetySetting>();
        ...
        internal class VendorGoogleChatTool
        {
            ...
            public VendorGoogleChatTool(Tool tool)
            {
                ...
                {
                    FunctionDeclarations ??= new List<VendorGoogleChatToolFunctionDeclaration>();
        ...
        public List<VendorGoogleChatRequestMessage> Contents { get; set; } = new List<VendorGoogleChatRequestMessage>();
        ...
        public static Tuple<List<VendorGoogleChatTool>?, VendorGoogleChatToolConfig?, VendorGoogleChatRequestGenerationConfig?> GetToolsAndToolChoice(ChatRequestResponseFormats? responseFormat, List<Tool>? tools, OutboundToolChoice? outboundToolChoice)
        {
            ...
            List<VendorGoogleChatTool>? localTools = new List<VendorGoogleChatTool>();
            ...
            foreach (Tool tool in tools)
            {
                if (tool.Function is not null)
                {
                    toolWrapper.FunctionDeclarations ??= new List<VendorGoogleChatToolFunctionDeclaration>();
            ...
            if (outboundToolChoice is not null)
            {
                ...
                switch (outboundToolChoice.Mode)
                {
                    ...
                    {
                        localToolConfig.FunctionConfig.Mode = "ANY";
                        localToolConfig.FunctionConfig.AllowedFunctionNames = new List<string> { outboundToolChoice.Function?.Name ?? string.Empty };
        ...
        public VendorGoogleChatRequest(ChatRequest request, IEndpointProvider provider)
        {
            ...
            GenerationConfig = new VendorGoogleChatRequestGenerationConfig
            {
                ...
                StopSequences = request.MultipleStopSequences is not null ? request.MultipleStopSequences.Take(5).ToList() : request.StopSequence is not null ? new List<string> { request.StopSequence } : null,
            ...
            if (request.Modalities?.Count > 0 && request.Model is not null)
            {
                ...
                {
                    GenerationConfig.ResponseModalities ??= new List<string>();
            ...
            else
            {
                if (request.ResponseFormat?.Type is ChatRequestResponseFormatTypes.StructuredJson or ChatRequestResponseFormatTypes.Json)
                {
                    ...
                    Tuple<List<VendorGoogleChatTool>?, VendorGoogleChatToolConfig?, VendorGoogleChatRequestGenerationConfig?> configUpdate = GetToolsAndToolChoice(request.ResponseFormat, request.ResponseFormat.Type is ChatRequestResponseFormatTypes.Json ? null : new List<Tool>
                    {
            ...
            if (request.VendorExtensions?.Google is not null)
            {
                ...
                {
                    Tools ??= new List<VendorGoogleChatTool>();
                ...
                if (request.VendorExtensions.Google.SafetyFilters is not null)
                {
                    ...
                    {
                        SafetySettings = new List<VendorGoogleChatRequestSafetySetting>();
