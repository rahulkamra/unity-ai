    ...
    internal static class Clr
    {
        ...
        private static TAttribute? GetCachedCustomAttribute<TAttribute>(ICustomAttributeProvider provider) where TAttribute : Attribute
        {
            return (TAttribute?)customAttributeCache.GetOrAdd((provider, typeof(TAttribute)), key => 
            {
                return key.Provider switch
                {
                    ...
                    _ => (key.Provider.GetCustomAttributes(typeof(TAttribute), false) as Attribute[] ?? Array.Empty<Attribute>()).FirstOrDefault() as TAttribute
                };
            });
        }
        ...
        private static object CreateCollection(Type dataType, Type elementType, IList intermediateValues)
        {
            ...
            catch (MissingMethodException)
            {
                ...
                MethodInfo? addMethod = GetCachedMethod(concreteType, "Add", BindingFlags.Public | BindingFlags.Instance, new Type[] { elementType });
                addMethod ??= GetCachedMethod(concreteType, "TryAdd", BindingFlags.Public | BindingFlags.Instance, new Type[] { elementType });

                if (addMethod is not null)
                {
                    foreach (object? listItem in intermediateValues)
                    {
                        addMethod.Invoke(finalCollection, new object[] { listItem });
                    }
                }
                else
                {
        ...
        public static async ValueTask<MethodInvocationResult> Invoke(Delegate? function, DelegateMetadata? metadata, string? data)
        {
            ...
            try
            {
                ...
                else if (invocationResult is not null)
                {
                    ...
                    {
                        Task asTask = (Task)type.GetMethod("AsTask").Invoke(invocationResult, null);
                        await asTask.ConfigureAwait(false);
                        PropertyInfo resultProperty = asTask.GetType().GetProperty("Result");
                    ...
                    if (paramType is ToolParamAwaitable awaitableParam)
                    {
                        ...
                        if (dataType!.IsGenericType)
                        {
                            if (dataType.GetGenericTypeDefinition() == typeof(Task<>))
                            {
                                ...
                                return fromResult.Invoke(null, new object[] { innerValue });
                            }
