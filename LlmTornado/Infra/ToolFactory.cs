...
namespace LlmTornado.Infra
{
    ...
    {
        public static readonly HashSet<ChatModel> ModelsSupportingStrictFunctions = new HashSet<ChatModel>
        {
        ...
        private static TAttribute? GetCachedCustomAttribute<TAttribute>(ICustomAttributeProvider provider) where TAttribute : Attribute
        {
            return (TAttribute?)customAttributeCache.GetOrAdd((provider, typeof(TAttribute)), key => 
            {
                return key.Provider switch
                {
                    ...
                    _ => (key.Provider.GetCustomAttributes(typeof(TAttribute), false) as Attribute[] ?? Array.Empty<Attribute>()).OfType<TAttribute>().FirstOrDefault()
                };
            });
        }
        ...
        {
            function.Params ??= new List<ToolParam>();
        ...
        public static DelegateMetadata CreateFromMethod(Delegate del, ToolMetadata? metadata, IEndpointProvider provider)
        {
            ...
            ToolDefinition function = new ToolDefinition
            {
                ...
                Params = new List<ToolParam>()
            };

            HashSet<string> ignored = metadata?.Ignore?.ToHashSet() ?? new HashSet<string>();
        ...
        public static IToolParamType GetParamFromType(Delegate? del, Type type, string? description, ParameterInfo? par, PropertyInfo? prop, int recursionLevel, Type topLevelType, IEndpointProvider provider)
        {
            ...
            if (baseType == typeof(JsonElement) || baseType.IsSubclassOf(typeof(JToken)) || baseType == typeof(JToken))
            {
                ...
                {
                    return new ToolParamObject(description, new List<ToolParam>())
                    {
                        DataType = type, 
            ...
            if (baseType.IsArray)
            {
                if (baseType.GetArrayRank() > 1)
                {
                    ...
                    return new ToolParamObject("A multi-dimensional array.", new List<ToolParam>
                    {
            ...
            {
                List<IToolParamType> itemTypes = new List<IToolParamType>();
                Type[] genericArgs = baseType.GetGenericArguments();
            ...
            ToolParamObject obj = new ToolParamObject(description, new List<ToolParam>()) { DataType = type, Serializer = ToolParamSerializer.Object };
