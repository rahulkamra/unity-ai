    ...
    public sealed class SseParser<T>
    {
        ...
        private bool ProcessLine(out SseItem<T> sseItem, out int advance)
        {
            ...
            else
            {
                ...
                fieldName = line;
                fieldValue = ReadOnlySpan<byte>.Empty;
            }

            if (fieldName.SequenceEqual(System.Text.Encoding.UTF8.GetBytes("data")))
            {
            ...
            else if (fieldName.SequenceEqual(System.Text.Encoding.UTF8.GetBytes("event")))
            {
                // Spec: "Set the event type buffer to field value."
                _eventType = SseParser.Utf8GetString(fieldValue);
            }
            else if (fieldName.SequenceEqual(System.Text.Encoding.UTF8.GetBytes("id")))
            {
            ...
            else if (fieldName.SequenceEqual(System.Text.Encoding.UTF8.GetBytes("retry")))
            {
