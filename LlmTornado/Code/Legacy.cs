...
namespace LlmTornado.Code
{
    ...
    internal static class HttpEncoder
    {
        ...
        private static readonly char[] s_htmlAttributeEncodingChars = new char[] { '<', '"', '\'', '&' };
        ...
        private static class UriUtil
        {
            internal static bool TrySplitUriForPathEncode(string input, out string schemeAndAuthority, out string path, out string queryAndFragment)
            {
                ...
                if (authorityEnd < 0)
                {
                    int queryStartNoPath = input.IndexOfAny(new char[] { '?', '#' }, schemeEnd + 3);
                ...
                int queryStart = rest.IndexOfAny(new char[] { '?', '#' });
                if (queryStart >= 0)
                {
